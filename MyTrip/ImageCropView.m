...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView  = [[ImageCropView alloc] initWithFrame:view blurOn:self.blurredBackground cropArea:_cropArea];
...
- (IBAction)done:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(ImageCropViewControllerDidEdit:didFinishCroppingImage:)])
    {
        UIImage *cropped;
        if (self.image != nil){
        ...
        [self.delegate ImageCropViewControllerDidEdit:self didFinishCroppingImage:cropped];
    }
...
- (id)initWithFrame:(CGRect)frame cropArea:(CGSize) area
{
    self = [super initWithFrame:frame];
    if (self) {
        [self initViews:area];
...
- (id)initWithFrame:(CGRect)frame blurOn:(BOOL)blurOn cropArea:(CGSize) area
{
    self = [super initWithFrame:frame];
    if (self) {
        self.blurred = blurOn;
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    if (self=[super initWithCoder:aDecoder]) {
		...
		[self initViews: defaultSize];
    }
    return self;
}

- (void)initViews:(CGSize) area {
	...
	int initialWidth = area.width / 2;
	int initialHeight = area.height / 2;
    CGPoint centerInView = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2);
    topLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialWidth,
                                                                   centerInView.y - initialHeight,
                                                                   controlPointSize)];
    
    bottomLeftPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x - initialWidth,
                                                                      centerInView.y + initialHeight,
                                                                      controlPointSize)];
    
    bottomRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialWidth,
                                                                       centerInView.y + initialHeight, controlPointSize) ];
    
    topRightPoint = [self createControlPointAt:SquareCGRectAtCenter(centerInView.x + initialWidth,
                                                                    centerInView.y - initialHeight, controlPointSize)];
